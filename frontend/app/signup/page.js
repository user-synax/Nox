...
export default function SignupPage() {
  ...
  const oauthMessage = oauthError
    ? oauthError.code === "access_denied"
      ? "Google sign-up was cancelled. Try again when you're ready."
      : oauthError.detail || "Google sign-up couldn't be completed. Try again."
    : null;
        ...
        >
          ...
          <form onSubmit={onSubmit} noValidate className="flex flex-col gap-4">
            ...
            <div className={`t-input-wrap ${username.error ? "is-error" : ""}`}>
              ...
              <div
                key={`username-${username.shakeKey}`}
                className={inputShell(username)}
              >
                <input
                  ref={usernameRef}
                  id="signup-username"
          ...
          <p className="mt-6 text-center text-[12px] leading-[1.2] tracking-[-0.12px] text-ink-muted">
            By continuing you agree to the Terms and Privacy Policy.
          </p>
          <p className="mt-3 text-center text-[14px] tracking-[-0.14px] text-ink-muted">
            Have an account?{" "}
            <Link
              href="/login"
